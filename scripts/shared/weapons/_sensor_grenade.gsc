	...
	foreach( player in level.players )
	{
		if ( player util::IsEnemyPlayer( self.owner ) )
		{
			if ( ! ( player hasPerk ("specialty_nomotionsensor") ) && 
			     ! ( player hasPerk ("specialty_sengrenjammer") && player clientfield::get( CLIENT_FIELD_FLAG_SG_JAMMER_ACTIVE ) )  )
			{
				if ( DistanceSquared( player.origin, origin ) < SENSOR_GRENADE_LASER_DISTANCE_SQ ) 
				{
...
function isPlayerTracked( player, time ) 
{
	...
	{
		if ( self.sensorGrenadeData[player.clientid] + SENSOR_GRENADE_TRACK_TIME > time )
		{
			playerTracked = true;
		}
	}
...
function watch_for_decoys( owner )
{
	...
	foreach( player in level.players )
	{
		if ( player util::IsEnemyPlayer( self.owner ) )
		{
			...
			{
				if ( DistanceSquared( player.origin, self.origin ) < DECOY_SPAWN_RADIUS * DECOY_SPAWN_RADIUS )
				{
...
function get_decoy_spawn_loc() 
{
	...
	return self.origin - DECOY_SPAWN_DISTANCE * AnglesToForward(self.angles);
}
