...
function __init__()
{
	clientfield::register( "toplayer", "empd", VERSION_SHIP, 1, "int" );
	clientfield::register( "toplayer", "empd_monitor_distance", VERSION_SHIP, 1, "int" );
...
function applyEMP( attacker, explosionpoint )
{
	...
	else
	{
		...
		ratio = 1 - ( distanceToExplosion / EMP_RADIUS );
		
		currentEmpDuration = EMP_MIN_TIME + ( ( EMP_MAX_TIME - EMP_MIN_TIME ) * ratio );
	}
...
function watchEMPExplosion( owner, weapon ) // self == grenade
{
	...
	level empExplosionDamageEnts( owner, weapon, origin, EMP_RADIUS, true );
}
