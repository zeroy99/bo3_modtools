...
#define MAX_SIMULTANEOUS_LOCKONS	GetDvarInt( "scr_max_simLocks" )			//#define MAX_SIMULTANEOUS_LOCKONS	5 this number corresponds to a code define

REGISTER_SYSTEM( "multilockap_guidance", &__init__, undefined )
...
function APLockLoop(weapon)
{
	...
	for (;;)
	{
		...
		if (self.multiLockList.size>=MAX_SIMULTANEOUS_LOCKONS || self.multiLockList.size>=self GetWeaponAmmoClip(weapon))
		{
			continue;
		}
		...
		if (IsDefined(bestTarget) && self.multiLockList.size<MAX_SIMULTANEOUS_LOCKONS && self.multiLockList.size<self GetWeaponAmmoClip(weapon))
		{
...
function GetBestTarget(weapon)
{
	...
	foreach(dot in dots)
	{
		...
		newEntry.apSoundId = RandomInt(INT_MAX);
		return newEntry;
	}
