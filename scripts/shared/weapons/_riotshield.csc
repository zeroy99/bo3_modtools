...
{
	clientfield::register( "scriptmover", "riotshield_state", VERSION_SHIP, 2, "int",&shield_state_change, !CF_HOST_ONLY, !CF_CALLBACK_ZERO_ON_NEW_ENT );
...
function shield_state_change( localClientNum, oldVal, newVal, bNewEnt, bInitialSnap, fieldName, bWasTimeJump )
{
	...
	switch( newVal )
	{
		case RIOTSHIELD_STATE_DEPLOYED:
		{
			instant = ( oldVal == RIOTSHIELD_STATE_DESTROYED );
			self thread riotshield_deploy_anim( localClientNum, instant );
			break;
		}
		case RIOTSHIELD_STATE_DESTROYED:
		{
