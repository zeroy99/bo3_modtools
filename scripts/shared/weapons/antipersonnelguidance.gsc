...
#define MAX_SIMULTANEOUS_LOCKONS	1	//this number corresponds to a code define

REGISTER_SYSTEM( "singlelockap_guidance", &__init__, undefined )
...
function APLockLoop(weapon)
{
	...
	for (;;)
	{
		...
		if (self.multiLockList.size>=MAX_SIMULTANEOUS_LOCKONS)
		{
			continue;
		}
		...
		if (IsDefined(bestTarget) && self.multiLockList.size<MAX_SIMULTANEOUS_LOCKONS)
		{
