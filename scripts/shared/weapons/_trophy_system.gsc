...
function register()
{
	clientfield::register( "missile", "trophy_system_state", VERSION_SHIP, 2, "int" );
	clientfield::register( "scriptmover", "trophy_system_state", VERSION_SHIP, 2, "int" );
}
...
function createTrophySystemWatcher() // self == player
{
	...
	watcher.activationDelay = TROPHY_ACTIVATION_DELAY_SECONDS;
	...
	watcher.stunTime = TROPHY_STUN_TIME_SECONDS;
}
...
function onTrophySystemSpawn( watcher, player ) // self == trophy system
{
	...
	if ( moveState == "rolling" )
	{
		...
		self clientfield::set( "trophy_system_state", TROPHY_SYSTEM_ROLLING );
	...
	self clientfield::set( "trophy_system_state", TROPHY_SYSTEM_STATIONARY );
...
function setReconModelDeployed() // self == trophy system
{
	...
	{
		self.reconModelEntity clientfield::set( "trophy_system_state", TROPHY_SYSTEM_STATIONARY );
	}
}
...
function trophyWatchHack() // self == trophy system
{
	...
	self clientfield::set( "trophy_system_state", TROPHY_SYSTEM_INIT );
}
	...
	while( true )
	{
		...
		for ( index=0; index < level.missileEntities.size; index++ )
		{
			...
			if ( isdefined( grenade.owner ))
			{
				...
				if ( grenadeDistanceSquared < ( TROPHY_ACTIVE_RADIUS * TROPHY_ACTIVE_RADIUS ))
				{
					if ( BulletTracePassed( grenade.origin, self.origin + (0,0,TROPHY_TRACE_Z), false, self, grenade, false, true ) )
					{
						playFX( level.trophyLongFlashFX, self.origin + (0,0,TROPHY_FLAK_Z), ( grenade.origin - self.origin ), AnglesToUp( self.angles ) );
...
function projectileExplode( projectile, trophy ) // self == trophy owning player
{
	...
	self challenges::trophy_defense( projPosition, TROPHY_ACTIVE_RADIUS );
...
function watchTrophySystemDamage( watcher ) // self == trophy system
{
	...
	damageMax = TROPHY_SYSTEM_HEALTH;
