...
function register()
{
	clientfield::register( "scriptmover", "riotshield_state", VERSION_SHIP, 2, "int" );
}
...
function watchRiotshieldDeploy() // self == player
{
	...
	if ( deploy_attempt )
	{
		...
		if ( placement["result"] /*&& riotshieldDistanceTest( placement["origin"] )*/)
		{
			...
			shield_ent clientfield::set( "riotshield_state", RIOTSHIELD_STATE_DEPLOYED );
			shield_ent.reconModel  clientfield::set( "riotshield_state", RIOTSHIELD_STATE_DEPLOYED );
...
function damageThenDestroyRiotshield( attacker, weapon ) // self == riotshield script_model ent
{
	...
	self NotSolid();
	self clientfield::set( "riotshield_state", RIOTSHIELD_STATE_DESTROYED );
