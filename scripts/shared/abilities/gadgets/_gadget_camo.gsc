...
function __init__()
{
	ability_player::register_gadget_activation_callbacks( GADGET_TYPE_OPTIC_CAMO, &camo_gadget_on, &camo_gadget_off );
	ability_player::register_gadget_possession_callbacks( GADGET_TYPE_OPTIC_CAMO, &camo_on_give, &camo_on_take );
	ability_player::register_gadget_flicker_callbacks( GADGET_TYPE_OPTIC_CAMO, &camo_on_flicker );
	ability_player::register_gadget_is_inuse_callbacks( GADGET_TYPE_OPTIC_CAMO, &camo_is_inuse );
	ability_player::register_gadget_is_flickering_callbacks( GADGET_TYPE_OPTIC_CAMO, &camo_is_flickering );
	
	clientfield::register( "allplayers", "camo_shader", VERSION_SHIP, 3, "int" );
...
function camo_on_spawn()
{
	...
	self clientfield::set( "camo_shader", GADGET_CAMO_SHADER_OFF );
	if ( IsDefined( self.sound_ent ) )
	{
		self.sound_ent stoploopsound( .05 );
		self.sound_ent delete();	
...
function suspend_camo_suit( slot, weapon )
{
	...
	self clientfield::set( "camo_shader", GADGET_CAMO_SHADER_FLICKER );

	suspend_camo_suit_wait( slot, weapon );

	if ( self camo_is_inuse( slot ) )
	{
		self clientfield::set( "camo_shader", GADGET_CAMO_SHADER_ON );
	}
}
