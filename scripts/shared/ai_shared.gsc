...
function shoot_at_target( mode, target, tag, duration, setHealth, ignoreFirstShotWait )
{
	...
	mode_flag = IS_EQUAL( mode, "normal" ) || IS_EQUAL( mode, "shoot_until_target_dead" ) || IS_EQUAL( mode, "kill_within_time" );
	...
	if ( !isdefined( target ) || ( ( IS_EQUAL( mode, "shoot_until_target_dead" ) ) && ( target.health <= 0 ) ) )
	{
...
function bloody_death( n_delay, hit_loc )
{
	...
	if( IS_TRUE( self.__bloody_death ) )
		return;
	...
	{
		assert( IsInArray( HITLOC_ALL, hit_loc ), "bloody_death() : Invalid hit location. Check the list of allowed hit location in blackboard.gsh, HITLOC_ALL." );
	}
	else
	{
		hit_loc = array::random( HITLOC_ALL );
	}
