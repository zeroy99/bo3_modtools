...
function add_interrupt_connection( from_state_name, to_state_name, on_notify, checkfunc )
{
	...
	connection.type = CONNECTION_TYPE_NOTIFY;
...
function add_utility_connection( from_state_name, to_state_name, checkfunc, defaultScore )
{
	...
	connection = SpawnStruct();
	connection.to_state = to_state;
	connection.type = CONNECTION_TYPE_UTILITY;
	connection.checkfunc = checkfunc;
	connection.score = defaultScore;
	if ( !isdefined( connection.score ) )
	{
		connection.score = DEFAULT_CONNECTION_SCORE;
	}

	ARRAY_ADD( from_state.connections_utility, connection );
...
function threadNotifyConnections( state )
{
	...
	{
		assert( connection.type == CONNECTION_TYPE_NOTIFY );
...
function connection_on_notify( state_machine, notify_name, connection )
{
	...
	while ( 1 )
	{
		...
		params = SpawnStruct();
		params.notify_param = [];
		ARRAY_ADD( params.notify_param, param0 );
		ARRAY_ADD( params.notify_param, param1 );
		ARRAY_ADD( params.notify_param, param2 );
		ARRAY_ADD( params.notify_param, param3 );
		ARRAY_ADD( params.notify_param, param4 );
		ARRAY_ADD( params.notify_param, param5 );
		ARRAY_ADD( params.notify_param, param6 );
		ARRAY_ADD( params.notify_param, param7 );
		ARRAY_ADD( params.notify_param, param8 );
		ARRAY_ADD( params.notify_param, param9 );
		ARRAY_ADD( params.notify_param, param10 );
		ARRAY_ADD( params.notify_param, param11 );
		ARRAY_ADD( params.notify_param, param12 );
		ARRAY_ADD( params.notify_param, param13 );
		ARRAY_ADD( params.notify_param, param14 );
		ARRAY_ADD( params.notify_param, param15 );
...
function evaluate_connections( eval_func, params )
{
	...
	{
		assert( connection.type == CONNECTION_TYPE_UTILITY );
		...
		if ( score > 0 )
		{
			ARRAY_ADD( connectionArray, connection );
			ARRAY_ADD( scoreArray, score );
