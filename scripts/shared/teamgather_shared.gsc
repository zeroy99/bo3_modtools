...
#define	DISPLAY_SUCCESS_TEXT_TIME		(0)			// 2
...
#define GATHER_RADIUS					(42*5)						// 42*4
#define TELEPORT_RADIUS					( GATHER_RADIUS - 42 )
#define GATHER_COUNTDOWN				10							// 10
#define	MIN_PLAYER_TELEPORT_DISTANCE	(42*2)

#define GATHER_FLOOR_EFFECT				"_t6/misc/fx_ui_flagbase_pmc"

#define WAYPOINT_SHADER					"T7_hud_prompt_press_64"		// "t7_hud_waypoints_beacon"
#define COMPASS_SHADER					"T7_hud_prompt_press_64"		// "t7_hud_minimap_beacon_key"

#precache( "material", WAYPOINT_SHADER );
#precache( "material", COMPASS_SHADER );
...
class cTeamGather
{
	...
	{
		if( DISPLAY_SUCCESS_TEXT_TIME > 0 )
		{
			x_off = 0;
			y_off = 180;
			...
			wait( DISPLAY_SUCCESS_TEXT_TIME );
	...
	function setup_gameobject( v_pos, STR_MODEL, STR_USE_HINT, e_los_ignore_me )
	{
		...
		e_object gameobjects::set_3d_icon( "friendly", WAYPOINT_SHADER );
		e_object gameobjects::set_3d_icon( "enemy", WAYPOINT_SHADER );
		e_object gameobjects::set_2d_icon( "friendly", COMPASS_SHADER );
		e_object gameobjects::set_2d_icon( "enemy", COMPASS_SHADER );
	...
	{
		if( !IS_TRUE(FLOOR_EFFECTS_ACTIVE) )
		{
			return;
		}
		...
		m_gather_fx = spawnFx( GATHER_FLOOR_EFFECT, v_floor_pos );
	...
	{
		start_player_timer( GATHER_COUNTDOWN );
	...
	function teamgather_main_update()
	{
		...
		{
			e_player = a_players[i];
			if( IS_TRUE(e_player.in_gather_position) )
			{
	...
	function update_players_in_radius( force_player_into_position )
	{
		...
		for( i=0; i<a_players.size; i++ )
		{
			...
			if( IS_TRUE(force_player_into_position) && !IS_TRUE(e_player.in_gather_position) )
			{
	...
	function teleport_player_into_position( e_player )
	{
		...
		while( 1 )
		{
			x_offset =  RandomFloatRange( -TELEPORT_RADIUS, TELEPORT_RADIUS );
			y_offset =  RandomFloatRange( -TELEPORT_RADIUS, TELEPORT_RADIUS );
			...
			for( i=0; i<a_players.size; i++ )
			{
				if( e_player != a_players[i] )
				{
					...
					if( dist < MIN_PLAYER_TELEPORT_DISTANCE )
					{
						reject = true;
						break;
					}
	...
	function get_time_remaining_in_seconds()
	{
		...
		time_remaining += 1;
		if( time_remaining > GATHER_COUNTDOWN )
		{
			time_remaining = GATHER_COUNTDOWN;
		}
...
function setup_teamgather( v_interact_pos, v_interact_angles, e_interact_entity )
{
	...
	v_gather_pos = v_interact_pos + ( v_forward * GATHER_POSITION_OFFSET );
