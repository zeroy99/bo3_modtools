...
{
	// INIT BLACKBOARD
	spawner::add_archetype_spawn_function( ARCHETYPE_HUMAN, &ArchetypeHumanBlackboardInit );
	spawner::add_archetype_spawn_function( ARCHETYPE_HUMAN, &ArchetypeHumanInit );
...
function private ArchetypeHumanInit()
{
	...
	if ( IS_TRUE( entity.hero ) )
	{
		Blackboard::SetBlackBoardAttribute( entity, HUMAN_LOCOMOTION_VARIATION, "alt1" );
	}
}
...
function private ArchetypeHumanOnBehaveCallback( entity )
{
	...
	{
		Blackboard::SetBlackBoardAttribute( entity, PREVIOUS_COVER_MODE, COVER_ALERT_MODE );
		Blackboard::SetBlackBoardAttribute( entity, COVER_MODE, COVER_MODE_NONE );
	}
	
	// Prevent an AI that just left anim script from throwing a grenade.
	grenadeThrowInfo = SpawnStruct();
	grenadeThrowInfo.grenadeThrower = entity;
	Blackboard::AddBlackboardEvent(
		"human_grenade_throw",
		grenadeThrowInfo,
		RandomIntRange( MIN_GRENADE_ANIMSCRIPTED_TIME, MAX_GRENADE_ANIMSCRIPTED_TIME ) );
}
	...
	inflictor, attacker, damage, meansOfDeath, weapon, dir, hitLoc, offsetTime )
{
	...
	if ( IsDefined( weapon.weapclass ) && weapon.weapclass == "turret" )
	{
		...
		if ( IsDefined( inflictor ) )
		{
			...
			isCloseExplosive = DistanceSquared( inflictor.origin, entity.origin ) <= CLOSE_EXPLOSIVE;
	...
	if ( forceGibbing || isExplosive ||
		...
		( weapon.doGibbing && ( attackerDistance <= SQR( weapon.maxGibDistance ) ) ) )
	{
