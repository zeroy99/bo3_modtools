...
REGISTER_SYSTEM( "killcam", &__init__, undefined )
	
function __init__()
{
	callback::on_start_gametype( &init );
	// clientuimodels are registered client-side in raw/ui/uieditor/clientfieldmodels.lua
	clientfield::register( "clientuimodel", "hudItems.killcamAllowRespawn", VERSION_SHIP, 1, "int" );
}
...
)
{
	...
	{
		/#
			println( "[KILLCAM] more than one client entering killcam this frame" );
		#/
...
function ended_killcam_cleanup()
{
	...
	// spawn at an intermission point
	self [[level.spawnIntermission]]( false );
}
...
function get_killcam_entity( attacker, eInflictor, weapon )
{
	...
	if ( isdefined( attacker ) && IsPlayer( attacker ) && attacker IsRemoteControlling() && ( eInflictor.controlled === true || eInflictor.occupied === true ) )
	{
		if ( ( weapon.name == "sentinel_turret" )
			|| ( weapon.name == "helicopter_gunner_turret_primary" )
			|| ( weapon.name == "helicopter_gunner_turret_secondary" )
			|| ( weapon.name == "helicopter_gunner_turret_tertiary" )
			|| ( weapon.name == "amws_gun_turret_mp_player" )
			|| ( weapon.name == "auto_gun_turret" ) )
			return undefined;
	}
