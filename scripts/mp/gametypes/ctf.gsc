...
REGISTER_SYSTEM( "ctf", &__init__, undefined )
	
function __init__()
{
	clientfield::register( "scriptmover", "ctf_flag_away", VERSION_SHIP, 1, "int" );	
}
...
function onSpawnPlayer(predictedSpawn)
{
	...
	self clientfield::set( CLIENT_FIELD_CTF_CARRIER, 0 );	
...
function ctf()
{
	...
	{
		/#print("Not enough ctf_flag_pickup_trig triggers found in map.  Need two.");#/
		return;
	}
	...
	{
		/#print("Not enough ctf_flag_zone_trig triggers found in map.  Need two.");#/
		return;
	}
...
function onDrop( player )
{
	...
	if ( isdefined( player ) )
	{
		player clientfield::set( CLIENT_FIELD_CTF_CARRIER, 0 );	
...
function onPickup( player )
{
	...
	{	
		...
		else if (distancesquared(self.trigger.baseOrigin, player.origin) > OBJECTIVE_FLAG_RETURN_SCORE_DIST*OBJECTIVE_FLAG_RETURN_SCORE_DIST) 
		{
		...
		self gameobjects::set_flags( OBJECTIVE_FLAG_AT_BASE );
	...
	else
	{
		...
		player clientfield::set( CLIENT_FIELD_CTF_CARRIER, 1 );	
		self gameobjects::set_flags( OBJECTIVE_FLAG_AWAY );
...
function onCapture( player )
{
	...
	if ( (level.flagCaptureCondition == CAPTURE_CONDITION_AT_BASE) && playerTeamsFlag gameobjects::is_object_away_from_home() )
	{
		return;
	}
